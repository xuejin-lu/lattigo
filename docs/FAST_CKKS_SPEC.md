...
| Fast polynomial execution | Implemented for the bounded single-polynomial CKKS path required by EvalMod | `circuits/ckks/polynomial/fast.go` evaluates Chebyshev polynomials with Fast q0/q1 arithmetic, reusable power/baby-step workspace, and the existing Paterson-Stockmeyer planner; EvalMod integration remains deferred |
...
| Fast ModDown, KeySwitch, EvalMod, and full Bootstrap execution | Not implemented | Fast ModUp basis raise, Trace, and ScaleDown boundaries are implemented, but EvalMod integration and full Fast Bootstrap execution remain deferred |
